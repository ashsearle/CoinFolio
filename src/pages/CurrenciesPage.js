...
class CurrenciesPage extends Component {
  ...
  render() {
    const data = this.props.currencies.map((currency) => {
      currency.market_cap = currency['market_cap_' + this.props.user.currency.toLowerCase()];
      currency.price = currency['price_' + this.props.user.currency.toLowerCase()];
      currency['24h_volume'] = currency['24h_volume_' + this.props.user.currency.toLowerCase()];
      return currency;
    });
    const columns = [{
      title: 'Rank',
      ...
      render: (text, record) => {
        return formatNumber(this.props.user, record.total_supply) + ' ' + record.symbol;
      }
    }, {
      title: 'Change (1h)',
      ...
      sorter: (a, b) => a.percent_change_24h_class - b.percent_change_24h_class,
      render: (text) => {
        return this.formatPercentChange(text);
      }
    }, {
